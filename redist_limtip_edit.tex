...
We construct three redistrubution scenarios based on different guiding
...
age members. Second, we redistribute conditional on the time available
to people. Finally, in the third scenario, redistribution is guided
...
household members that are not of working age. Thus, the goal of the
different scenarios is to simulate different \(\alpha_{ij}\) values,
which represent the share of required household production time that
each household member takes on. We outline the methods used for
...
contrast with Scenario 1, this scenario suggests that household
responsabilities could be redistributed relative to the available
individuals may have after setting aside the time for personal
maintenance requirements and income generation
(\(Z_{ij}=168-M-D_{ij}(L_{ij}+T_{ij}\)). In other words, household
members should split up the required household production time based on
the time each one has available, i.e based on an equity criteria.
...
(\(Z_{ij}<0\)), we set their time available to zero. This ensures that
