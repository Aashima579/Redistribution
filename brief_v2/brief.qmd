...
Between 2015 and 2019 in the U.S., under LIMTIP definition, an average of 36% <!-- add footnote estimates in this section correspond to households with atleast one eligible person in 18-64 and not disabled. -->
of all households had at least one member who was time poor.
...
For the implementation of these scenarios, we consider that all elements in LIMTIP @eq-bal remain constant with the exception of $\alpha_{ij}$, which is the share of household production time that each individual $i$ in Household $j$ takes on. The goal is to simulate different $\alpha_{ij}$ based on each redistribution scenario, but maintaining the total share of work done by the eligible household members (all members 18-64 <!--and not disabled-->).This approach imposes the implicit assumption that all household members are equally efficient at taking care of the household responsibilities. We outline the methods used for implementing the scenarios below.
